...
connection entity (`application/models/connection.py`) and that designates the engine's player by
its Discord identifier. The engine never has to know about sessions or cookies.
...
persistence, which is built today without it. Each caller therefore imports the module it needs:
    ...
    from engine.models.seats import Seats
