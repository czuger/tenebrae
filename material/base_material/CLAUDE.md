...
already lives in `tenebrae/game_box/`, which is the repository's source of truth — that is where
both code and documentation read.

We come back here only to **check or complete a transcription**, and then the derived file in
`tenebrae/game_box/` is updated: it is never the source that is edited, and it is never the source
that a piece of code reads.
