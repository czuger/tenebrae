...
else - the game, the player, the seating table - lives in `tenebrae/engine/models/`.
...
- `view.py`: where a player was on the map, and how close they had zoomed in. A Mongo document,
  written by `tenebrae/application/repositories/view.py`. It is here and not in the engine because
  the engine does not know that an image, pixels or a window exist: a game plays from an
  interpreter, where zoom means nothing.
