...
of the routes: `tenebrae/application/app.py` imports neither `tenebrae.engine.models` nor
`mongoengine`, it calls `load`, `save` and `new_game`, and that is all.
...
As in `tenebrae/engine/models/`, this file re-exports nothing: `tenebrae/application/app.py` imports
a branch only when its configuration asks for it, and an application mounted without persistence
must not load mongoengine for all that.
